...
def transloc(mycelia, num_total_segs, dtt, isActiveTrans, whichInitialCondition,
    ...
    K_cw = advection_vel_cw*params['dt'] #K_cw is now the maximum amount of material that can be transported in dt.
    K_gluc = advection_vel_gluc*params['dt']
    ...
    if (np.any(mycelia['cw_i'][:num_total_segs] + params['dt']*cw_convect_term < 0)):
        ...
        #breakpoint()
       # Update concentrations due to convection
    if (np.any(mycelia['gluc_i'][:num_total_segs] + params['dt']*gluc_convect_term < 0)):
        bad_idx = np.where((mycelia['gluc_i'][:num_total_segs] + params['dt']*gluc_convect_term[:num_total_segs]) < 0)
        print('Bad indices: ',bad_idx)
        print('Glucose before conversion:',mycelia['gluc_i'][bad_idx])
        print('Amount converted:',gluc_convect_term[bad_idx]*params['dt'])
        print('Convert rate:',gluc_convect_term[bad_idx])
        print('Glucose after conversion:',mycelia['gluc_i'][bad_idx] + params['dt']*(gluc_convect_term[bad_idx]))
        #breakpoint()
